...
* **Vi mode fixes**: `>>`/`<<` indent operators now work, including visual/visual-line/visual-block and `.` repeat (#2438, #2606); `i"`/`a"` now search forward on the line instead of requiring the cursor already inside the quotes (#2439); `a"` includes trailing whitespace, matching Vim's `:help aquote` (#2604); `j`/`k` clamp to the line's last character on shorter lines instead of overshooting by one (#2442); `cw` stops at word end instead of eating the trailing whitespace like `dw` (#2437); and the `f`/`t`/`F`/`T` find-char motions now work with operators (`df`, `ct`, …) instead of hanging in operator-pending mode or deleting the wrong span (#2441).
